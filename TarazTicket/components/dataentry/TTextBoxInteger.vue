<template>
  <validation-provider
    ...
    :rules="rules"
  >
    <b-input-group id="t-input-group-integer" size="sm">
      <template #append>
      ...
      <b-form-input
        id="t-form-input"
        :disabled="disabled"
        :value="hasComma ? $tnumber.integerCommas(value) : value"
        :state="validateState(validationContext)"
        autocomplete="off"
        @input="onInput"
        @keypress="$tnumber.acceptInteger"
      />
    </b-input-group>
    <p id="error-feedback">
...
export default {
  ...
  methods: {
    ...
    onInput (value) {
      if (this.hasComma) {
        const _value = this.$tnumber.removeCommas(
          this.$tnumber.convertPersianNumber(value)
        )
        this.$emit(
          'input',
          value == null || value === '' ? null : parseInt(_value)
        )
      } else {
        const _value = this.$tnumber.convertPersianNumber(value)
        this.$emit(
          'input',
          value == null || value === '' ? null : parseInt(_value)
        )
