...
//        }
    ...
    @Override
    public InputStreamResource exportCrystalReportToPDF(Class<T> clazz, String rptFileName, HashMap<String, Object> parameterFields, List<TParameter<?>> tParameters) {
...
//        List<T> list = execSPNativeQuery(clazz, storedProcedureName, tParameters, ShowSPReturnType.NATIVE);
        // قاعدتا با کد زیر نیز باید این متد بدرستی کار کند
        // ولی هنگام ساخت پی دی اف به مشکل برمیخورد
        // List<Map<String, Object>> rows = execSPNativeQuery(clazz, storedProcedureName, tParameters, ShowSPReturnType.MAP);
        HashMap<String,Object> hm=new HashMap<>();
        hm.put("voucherDetailID",new BigDecimal(1));
        ArrayList<HashMap<String,Object>> ar=new ArrayList<>();
        ar.add(hm);

        SaleCrystalRepVoucher saleCrystalRepVoucher=new SaleCrystalRepVoucher();
        saleCrystalRepVoucher.setVoucherDetailID(new BigDecimal(2));
        ArrayList<SaleCrystalRepVoucher> arrayList=new ArrayList<>();
        arrayList.add(saleCrystalRepVoucher);

        SaleVoucherDetail saleVoucherDetail=new SaleVoucherDetail();
        saleVoucherDetail.setVoucherDetailID(new BigDecimal(3));
        ArrayList<SaleVoucherDetail> arrayList1=new ArrayList<>();
        arrayList1.add(saleVoucherDetail);

        Gson gson = new Gson();
        JsonElement jsonElement = gson.toJsonTree(hm);
        ArrayList<JsonElement> arrayList2=new ArrayList<>();
        arrayList2.add(jsonElement);
        SaleCrystalRepVoucher pojo = gson.fromJson(jsonElement, SaleCrystalRepVoucher.class);


        final BeanGenerator beanGenerator = new BeanGenerator();
        final Map<String, Class<?>> properties = new HashMap<String, Class<?>>();
        properties.put("voucherHeaderID", BigDecimal.class);
        properties.put("voucherDetailID", BigDecimal.class);
        BeanGenerator.addProperties(beanGenerator, properties);
        Class clazzz = (Class<?> ) beanGenerator.createClass();
        Object instance = null;
        try {
            instance = clazzz.newInstance();
            Method setJobName = clazzz.getMethod("setVoucherHeaderID", BigDecimal.class); // Gets the setJobName method that takes one String argument
            setJobName.invoke(instance, new BigDecimal(8));
        } catch (InstantiationException e) {
            e.printStackTrace();
        } catch (IllegalAccessException e) {
            e.printStackTrace();
...
//                    if (f.getName().equals("$cglib_prop_VoucherHeaderID")) {
//                        f.set(instance, new BigDecimal(109));
//                    } else if (f.getName().equals("$cglib_prop_VoucherDetailID")) {
//                        f.set(instance, new BigDecimal(1009));
//                    }
...
//            reportClientDocument.getDatabaseController().setDataSource(ar, HashMap.class, storedProcedureNameInRptFile, storedProcedureNameInRptFile);
//            reportClientDocument.getDatabaseController().setDataSource(arrayList, SaleCrystalRepVoucher.class, storedProcedureNameInRptFile, storedProcedureNameInRptFile);
//            reportClientDocument.getDatabaseController().setDataSource(arrayList1, SaleVoucherDetail.class, storedProcedureNameInRptFile, storedProcedureNameInRptFile);
//            reportClientDocument.getDatabaseController().setDataSource(arrayList2, JsonElement.class, storedProcedureNameInRptFile, storedProcedureNameInRptFile);
            reportClientDocument.getDatabaseController().setDataSource(arrrr, instance.getClass(), storedProcedureNameInRptFile, storedProcedureNameInRptFile);
        } catch (ReportSDKException e) {
