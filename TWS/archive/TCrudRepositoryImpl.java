...
//    private String _storedProcedureName(Class<T> clazz) {
//        for (Annotation annotation : clazz.getDeclaredAnnotations()) {
//            if (annotation.annotationType() == javax.persistence.NamedStoredProcedureQuery.class) {
//                javax.persistence.NamedStoredProcedureQuery namedStoredProcedureQuery = (javax.persistence.NamedStoredProcedureQuery) annotation;
//                return namedStoredProcedureQuery.procedureName();
//            }
//        }
//        return null;
//    }
    ...
    @Override
    public ResultErrMsgNewID executeStoredProcedureInsUpd(Class<T> clazz, String storedProcedureName, List<TParameter<?>> tParams) {
        ...
        return new ResultErrMsgNewID(result, errMsg, newID, formID);
    }

    @Override
    public List showSPNativeQuery(Class<T> clazz, String storedProcedureName, List<TParameter<?>> tParams, ShowSPReturnType showSPReturnType) {
    ...
    @Override
    public List<Map<String, Object>> executeQuery(String sqlString) {
        Query query = entityManager.createNativeQuery(sqlString);
        NativeQueryImpl nativeQuery = (NativeQueryImpl) query;
        nativeQuery.setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE);
...
//        }
    ...
    private StoredProcedureQuery _StoredProcedureQuery(String storedProcedureName) {
        StoredProcedureQuery storedProcedureQuery = entityManager
                .createNamedStoredProcedureQuery(storedProcedureName);
    ...
    private Connection _connection() {
        ...
        return DataSourceUtils.getConnection(entityManagerFactoryInfo.getDataSource());
//        try {
//            return DataSourceUtils.getConnection(entityManagerFactoryInfo.getDataSource());
////            return entityManagerFactoryInfo.getDataSource().getConnection();
...
//        return null;
    ...
    private SQLServerCallableStatement _sqlServerCallableStatement(Connection connection, StoredProcedureQuery storedProcedureQuery, String storedProcedureName, List<TParameter<?>> tParameters) {

        List<Parameter<?>> parameters = new ArrayList<>(storedProcedureQuery.getParameters());
        // Create CALL Query
        ...
        try {
            ...
            for (int i = 0; i < parameters.size(); i++) {
                ProcedureParameterImpl parameter = (ProcedureParameterImpl) parameters.get(i);
                if (parameter.getMode() == ParameterMode.IN) {
                    Predicate<TParameter> byName = tParameter -> tParameter.getName().equals(parameter.getName());
                    TParameter<?> tParametersFiltered = tParameters.stream().filter(byName).findFirst().get();
                    if (parameter.getParameterType() == String.class) {
                        if (tParametersFiltered.getValue() == null) {
                            sQLServerCallableStatement.setNull(parameter.getName(), Types.NVARCHAR);
                        } else {
                            sQLServerCallableStatement.setString(parameter.getName(), (String) tParametersFiltered.getValue());
                        }
                    } else if (parameter.getParameterType() == BigDecimal.class) {
                        if (tParametersFiltered.getValue() == null) {
                            sQLServerCallableStatement.setNull(parameter.getName(), Types.DECIMAL);
                        } else {
                            sQLServerCallableStatement.setBigDecimal(parameter.getName(), (BigDecimal) tParametersFiltered.getValue());
                        }
                    } else if (parameter.getParameterType() == Boolean.class) {
                        if (tParametersFiltered.getValue() == null) {
                            sQLServerCallableStatement.setNull(parameter.getName(), Types.BIT);
                        } else {
                            sQLServerCallableStatement.setBoolean(parameter.getName(), (Boolean) tParametersFiltered.getValue());
                        }
                    } else if (parameter.getParameterType() == Date.class) {
                        if (tParametersFiltered.getValue() == null) {
                            sQLServerCallableStatement.setNull(parameter.getName(), Types.DATE);
                        } else {
                            Date date = (Date) tParametersFiltered.getValue();
                            SimpleDateFormat formatter = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
                            Date date1 = formatter.parse(formatter.format(date));
                            java.sql.Timestamp ts = new Timestamp(date1.getTime());

                            sQLServerCallableStatement.setTimestamp(parameter.getName(), ts);
//                            sQLServerCallableStatement.setNull(parameter.getName(), Types.DATE);
                        }
                    } else if (parameter.getParameterType() == Timestamp.class) {
                        if (tParametersFiltered.getValue() == null) {
                            sQLServerCallableStatement.setNull(parameter.getName(), Types.ROWID);
                        } else {
                            sQLServerCallableStatement.setTimestamp(parameter.getName(), (Timestamp) tParametersFiltered.getValue());
                        }
                    } else if (parameter.getParameterType() == Blob.class) {
                        sQLServerCallableStatement.setStructured(i + 1, tParametersFiltered.getType(), (ISQLServerDataRecord) tParametersFiltered.getValue());
                    }
                } else if (parameter.getMode() == ParameterMode.OUT) {
                    if (parameter.getParameterType() == String.class) {
                        sQLServerCallableStatement.registerOutParameter(parameter.getName(), java.sql.Types.NVARCHAR);
                    } else if (parameter.getParameterType() == BigDecimal.class) {
                        sQLServerCallableStatement.registerOutParameter(parameter.getName(), Types.DECIMAL);
                    } else if (parameter.getParameterType() == Boolean.class) {
                        sQLServerCallableStatement.registerOutParameter(parameter.getName(), Types.BIT);
                    } else if (parameter.getParameterType() == Date.class) {
                        sQLServerCallableStatement.registerOutParameter(parameter.getName(), Types.DATE);
                    } else if (parameter.getParameterType() == Timestamp.class) {
                        sQLServerCallableStatement.registerOutParameter(parameter.getName(), Types.TIMESTAMP);
                    }
                }
            }
        } catch (SQLException throwables) {
    ...
    private ResultSet _resultSet(SQLServerCallableStatement sqlServerCallableStatement) {
        ResultSet resultSet = null;
        try {
            resultSet = sqlServerCallableStatement.executeQuery();
    ...
    @Override
    public List<CrmQKeyExecuteQuery> executeIdCodeDescQuery(String queryString) {
        Query query = entityManager.createNativeQuery(queryString, CrmQKeyExecuteQuery.class);
        List<CrmQKeyExecuteQuery> list = query.getResultList();
        if (list.isEmpty()) {
            throw new TNoContentException();
        }
    ...
    @Override
    public ArrayList<HashMap<String, Object>> executeQuery(String storedProcedureName, List<TParameter<?>> tParameters) {
        // هر بار اس پی بصورت جی دی بی سی کال شود این 4 خط نیاز است
        Connection connection = _connection();
        StoredProcedureQuery storedProcedureQuery = _StoredProcedureQuery(storedProcedureName);
        SQLServerCallableStatement sqlServerCallableStatement = _sqlServerCallableStatement(connection, storedProcedureQuery, storedProcedureName, tParameters);
        ResultSet resultSet = _resultSet(sqlServerCallableStatement);
        //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

        // تبدیل ریزالت ست به ارری لیست
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                for (int j = 0; j < columnNames.length; j++) {
                    row.put(columnNames[j], resultSet.getObject(columnNames[j]));
                }
                rows.add(row);
            }
        } catch (SQLException throwables) {
    ...
    @Override
    public HashMap<String, Object> execute1(String storedProcedureName, List<TParameter<?>> tParameters) {
        //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        Connection connection = _connection();
        StoredProcedureQuery storedProcedureQuery = _StoredProcedureQuery(storedProcedureName);
        SQLServerCallableStatement sqlServerCallableStatement = _sqlServerCallableStatement(connection, storedProcedureQuery, storedProcedureName, tParameters);
        try {
            ...
            throw new TSqlException(sQLTimeoutException.getMessage());
        }
        //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

        // گرفتن پارامتر های خروجی اس پی
        HashMap<String, Object> outParameters = new HashMap();
        try {
            List<Parameter<?>> parameters = new ArrayList<>(storedProcedureQuery.getParameters());
            for (int i = 0; i < parameters.size(); i++) {
                ProcedureParameterImpl parameter = (ProcedureParameterImpl) parameters.get(i);
                if (parameter.getMode() == ParameterMode.OUT) {
                    if (parameter.getParameterType() == String.class) {
                        outParameters.put(parameter.getName(), sqlServerCallableStatement.getString(parameter.getName()));
                    }
                    if (parameter.getParameterType() == BigDecimal.class) {
                        outParameters.put(parameter.getName(), sqlServerCallableStatement.getBigDecimal(parameter.getName()));
                    }
                }
            }
        } catch (SQLException sQLException) {
    ...
    @Override
    public InputStreamResource exportCrystalReportToPDF1(Class<T> clazz, String rptFileName, HashMap<String, Object> parameterFields, List<TParameter<?>> tParameters) {
        ...
        List<T> list = showSPNativeQuery(clazz,storedProcedureName,tParameters,ShowSPReturnType.NATIVE);
////        List<Map<String, Object>> rows = _nativeQueryResultList(clazz, storedProcedureName, tParameters, false);
//        AccBank accBank = new AccBank();
////        accBank.setRecordCount(new BigDecimal(1));
//        accBank.setBankName("سلمان");
////        accBank.setBankId(new BigDecimal(1));
////        accBank.setServerID(new BigDecimal(2));
////        accBank.setModifyDate(new Date());
////        accBank.setModifierID(new BigDecimal(3));
////        accBank.setCreatorID(new BigDecimal(4));
////        accBank.setBankCode("bco");
////        accBank.setBankLatinName("majidi");
////        accBank.setCreateDate(new Date());
////        accBank.setTaxCode("tco");
//        ArrayList<AccBank> lis = new ArrayList();
//        lis.add(accBank);
//
//        HashMap<String, Object> hashMap = new HashMap();
////        hashMap.put("bankId",new BigDecimal(1));
//        hashMap.put("BankName", "salman");
////        hashMap.put("bankLatinName","majidi");
//
//        ArrayList<HashMap<String, Object>> banks = new ArrayList();
//        banks.add(hashMap);
    ...
    @Override
    public InputStreamResource exportCrystalReportToPDF(String rptFileName, HashMap<String, Object> parameterFields, List<TParameter<?>> tParameters) {
        ReportClientDocument reportClientDocument = _reportClientDocument(rptFileName);
        //گرفتن نام اس پی از فایل آر پی تی
        String storedProcedureNameInRptFile = null;
        try {
        ...
        // ست کردن دیتا سورس به ریپورت
        try {
            reportClientDocument.getDatabaseController().setDataSource(resultSet, storedProcedureNameInRptFile, storedProcedureNameInRptFile);
        } catch (ReportSDKException e) {
            throw new TCrystalReportDataSourceException(e.getMessage());
        }
