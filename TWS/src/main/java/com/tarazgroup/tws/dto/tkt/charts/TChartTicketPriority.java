...
import com.tarazgroup.tws.dto.tkt.routerParams.TRouterParamsPriority;
 ...
 * @since 2.0.0
 */
public class TChartTicketPriority extends TChart implements TRouting {

    public TChartTicketPriority(String xFieldName, String[] yFieldNames, String[] yFieldTitles, List<Map<String, Object>> rows) {
    ...
    public TRouter[] generateRoutes(String xFieldName, List<Map<String, Object>> rows) {
        ...
        for (int i = 0; i < length; i++) {
            TRouter tRouter = new TRouter(rows.get(i).get(xFieldName).toString(),
                    null,
                    "tickets",
                    new TRouterParamsPriority((BigDecimal) rows.get(i).get("PriorityID")),
                    null);
            routes[i] = tRouter;
        }
        return routes;
